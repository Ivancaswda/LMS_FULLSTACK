...
const Navbar = () => {
    ...
    const {isEducator,  backendUrl, setIsEducator, getToken} = useContext(LmsContext)
         ...
         lg:px-36 border-b border-gray-500 py-4 ${isCourseListPage ? 'bg-white' : 'bg-cyan-100/70'}`}>
            ...
            <div className='hidden md:flex items-center gap-5 text-gray-500'>
                <div className='flex items-center gap-2'>
                    { user && (
                        <>
                            <button onClick={becomeEducator} className='hover:underline'>{ !isEducator ? 'Стань преподователем' : 'Панель преподователя'}</button>
